...
public class AddCommand : IBotCommandWithState
{
            ...
            case UpdateType.Message:
                ...
                if (stateDict.TryGetValue(update.Message!.From!.Id, out var state))
                {
                    ...
                    if (await attributeInfo.CorrectFormateFunction(messageText))
                    {
                        ...
                        var addableWithRole = UserAttributesInfoManager.AddableAttributes
                            .Where(attr => state.AddUser.Role.ImplementsAttribute(attr)).ToArray();
                        ...
                        else
                        {
                            ...
                            await botHandler.SendResultMessage(new UserDomain[] { state.AddUser }, chat, userId,
                                Role.Counsellor, "");
                        }
        ...
        bool wasIncorrect)
    {
        ...
        ReplyMarkup markup = attributeInfo.KeyboardMarkup != null
            ? await attributeInfo.KeyboardMarkup(state.AddUser)
    ...
    private class AddState
    {
        ...
        public readonly UserDomain AddUser = new() { FullName = new FullName("", ""), PhoneNumber = "" };
